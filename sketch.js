...
function draw() {
  ...
  if (song.isPlaying()) {
  let scales = min(windowWidth / bg.width, windowHeight / bg.height);
  let newWidth = scales * bg.width;
  let newHeight = scales * bg.height;
  
  push();
  translate(-newWidth / 2, -newHeight / 2);  // Translate to center the image
  image(bg, 0, 0, newWidth, newHeight);
  pop();
    if (fileUpload == true) {
      push();
      textSize(10);
