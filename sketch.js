...
function preload() {
  ...
  bgImages[2] = loadImage("Image/Treadmill3.jpg");
...
function draw() {
  ...
  if (song.isPlaying()) {
  let ratio = bg.width / bg.height;  // Calculate the aspect ratio of the image
  let w, h;
  
  if (windowWidth / windowHeight > ratio) {  
    // The window width is proportionally larger than the height, so match the width
    w = windowWidth;
    h = windowWidth / ratio;
  } else {  
    // The window height is proportionally larger than the width, so match the height
    h = windowHeight;
    w = windowHeight * ratio;
  }
  
  push();
  translate(-w / 2, -h / 2);  // Translate to center the image
  image(bg, 0, 0, w, h);  // Draw the image with the calculated width and height.
  pop();
    if (fileUpload == true) {
      push();
      textSize(10);
